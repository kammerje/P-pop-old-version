...
sys.path.append('forecaster/forecaster')
...
class system():
	...
	def __init__(self,
		...
		# Stability check
		self.apply_stability_check = apply_stability_check
		# Correlation in radii and periods from Weiss et al
		self.apply_correlations = apply_correlations
...
#        mu = np.log(13.); zeta = 1.5 # clean (Ertel et al. 2018)
		...
		# We order the planets by orbital periods (so that we have them going from the innermost one to the outtermost)
		dummy = np.array(dummy)
		dummy[0] = dummy[0][dummy[1].argsort()]
		dummy[1] = dummy[1][dummy[1].argsort()]

		kernel_radius = scp_stats.gaussian_kde(kernel_datasets.radii)
		kernel_periods = scp_stats.gaussian_kde(kernel_datasets.periods)
		#kernel_separation = scp_stats.gaussian_kde(kernel_datasets.distances)
		# At this point we have the dummy array whose 0 element contains the values of radii and
		# the 1 element contains the value for orbital Period

		#If there is at least one planet in the system, we compute its mass using mrforecaster
		#And its eccentricity. Note that we are now drawing these parameters here instead of doing
		#it in the exoplanet class as Jens used to do because we need them earlier, this should
		#however not change anything



		if(self.nplanets == 1):
			planet_masses =  mr.Rpost2M(radius=dummy[0], unit='Earth', grid_size=1e3, classify='No')
			planet_ecc = np.zeros(self.nplanets)
		
		# Weiss et al radii correlation only happen if there is more than one planet 
		#If this is the case then we recompute the values for radii according to Weiss et al.
		elif(self.nplanets > 1):
			#dummy = Stats.weiss_radius_corr(dummy)
			#print("max before corr: " + str(max(dummy[0])) + " min : " + str(min(dummy[0])))
			...
			if(self.apply_correlations):
				...
				if(min(dummy[0]) < 0.1 or max(dummy[0]) > 100):#If the correlation created too extreme values
					while(min(dummy[0]) < 0.1 or max(dummy[0]) > 100):#We redraw it until it is inside the range
						#dummy = Stats.weiss_radius_corr_skew(dummy)
						dummy = Stats.weiss_radius_corr_kernel(dummy, kernel_radius)
			#We compute the masses after the radius/period correlation because mass depends on R
			planet_ecc = np.zeros(self.nplanets)
			#print("max after corr: " + str(max(dummy[0])) + " min : " + str(min(dummy[0])))
			planet_masses =  mr.Rpost2M(radius=dummy[0], unit='Earth', grid_size=1e3, classify='No')
			
			if(self.apply_stability_check):
				#We check if the system that we have created is stable
				self.stability = Stats.check_system_stability(self.Ms, dummy, planet_masses, planet_ecc, 8)
				
				max_trials = 25
				trials = 0
				while(self.stability == False and trials <= max_trials):# If it is unstable, we redraw new parameters
					dummy = []
					#We first redraw P and R according to the model we chose
					if (self.rates == 'sag13'):
						dummy = Stats.sag13(nplanets = self.nplanets)
					elif (self.rates == 'hsu2019'):
						dummy = Stats.hsu2019()
					elif (self.rates == 'dressing2015'):
						dummy = Stats.dressing2015(nplanets = self.nplanets)
					elif (self.rates == 'fressin2013'):
						dummy = Stats.fressin2013()
					elif (self.rates == 'sag13_dressing2015'):
						dummy = Stats.sag13_dressing2015(nplanets = self.nplanets)
					elif (self.rates == 'hsu2019_dressing2015'):
						dummy = Stats.hsu2019_dressing2015()
					...
					#Otherwise, we recompute correlations and stability
					#print("(redraw) max before corr: " + str(max(dummy[0])) + " min : " + str(min(dummy[0])))
					#dummy = Stats.weiss_radius_corr_skew(dummy)
					#dummy = Stats.weiss_period_corr(dummy)
					if(self.apply_correlations):
						dummy = Stats.weiss_radius_corr_kernel(dummy, kernel_radius)
					...
					#We compute the masses after the radius/period correlation because mass depends on R
					planet_ecc = np.zeros(self.nplanets)
					#print("(redraw) max after corr: " + str(max(dummy[0])) + " min : " + str(min(dummy[0])))
					planet_masses =  mr.Rpost2M(radius=dummy[0], unit='Earth', grid_size=1e3, classify='No')
				...
				if(trials == max_trials):
					self.nplanets = 0
		# Weiss et al. planet separation correlation only happen if there is more than 2 planets

		

		#if(self.nplanets > 2):
		#	dummy = Stats.weiss_separation_corr(dummy)
...
#            import pdb; pdb.set_trace()
		...
		for k in range(nplanets):
			...
			cell  = np.random.choice(len(np.concatenate(rates_extrap_norm)), p = np.concatenate(rates_extrap_norm))
...
#            import pdb; pdb.set_trace()
	...
	def sag13_dressing2015(self, nplanets = False):
		"""
		...
		if (self.stype in 'AFGK'):
			Rp, Porb = self.sag13(nplanets = nplanets)
		elif (self.stype in 'M'):
			Rp, Porb = self.dressing2015(nplanets = nplanets)
		else:
			Rp = []; Porb = []
	...
	"""
	Added part. Implemented for the master's thesis of Emile Fontanet, which aims
	to update some parts of the stellar catalogue as well as adding a few features
=================================================================================
	"""
	def weiss_radius_corr(self, dummy):
		"""
		Recomputes the planet radii in a multi-planet system according to the correlations
		found by Weiss et al. 2018
		(https://arxiv.org/abs/1706.06204)

		Parameters : 
		dummy : array of arrays, dummy[0] is the array with the radii and dummy[1] the array
		with the orbital periods.
		"""		

		#Now, dummy[0][0] is the radius of the planet which is the closest to the star
		#We now apply Weiss et al. statistics (for the time being I am just making
		#a normal dist around the value of the first planet and taking the abs value to remove
		#negative radii)
		mu = 1.14435351
		sigma = 0.42259806
		for i in range(len(dummy[0][1:])):
			radius_ratio = np.abs(np.random.normal(loc = mu, size = 1, scale = sigma))
			dummy[0][1 + i] = dummy[0][i]*radius_ratio
		dummy[0][1:] += (dummy[0][1:] <= 0.1)*0.1
		return dummy


	def weiss_radius_corr_skew(self, dummy):
		"""
		Recomputes the planet radii in a multi-planet system according to the correlations
		found by Weiss et al. 2018
		(https://arxiv.org/abs/1706.06204)
		...
		#a normal dist around the value of the first planet and taking the abs value to remove
		#negative radii)
		mu, skew, sigma = 0.69007816, 3.52948576, 0.70029624
		for i in range(len(dummy[0][1:])):
			radius_ratio = scp_stats.skewnorm(skew, loc = mu, scale = sigma).rvs()
			if(radius_ratio <= 0):
				print("There was a negative radius")
			dummy[0][1 + i] = dummy[0][i]*radius_ratio
		return dummy
	

	def weiss_radius_corr_kernel(self, dummy, kernel ):
		"""
		Recomputes the planet radii in a multi-planet system according to the correlations
		found by Weiss et al. 2018 using kernel density estimator pdf
		(https://arxiv.org/abs/1706.06204)

		Parameters : 
		dummy : array of arrays, dummy[0] is the array with the radii and dummy[1] the array
		with the orbital periods.
		kernel : the kernel density estimator pdf
		"""

		for i in range(len(dummy[0][1:])):
			radius_ratio = kernel.resample(1)[0]
			if(radius_ratio <= 0):
				print("There was a negative radius")
			dummy[0][1 + i] = dummy[0][i]*radius_ratio
		return dummy
	...
	def weiss_separation_corr(self, dummy):
		"""
		Recomputes the planet radii in a multi-planet system according to the correlations
		found by Weiss et al. 2018 using a normal distribution
		(https://arxiv.org/abs/1706.06204)
		...
		mu, sigma = 0.98172817, 0.20175955 #Parameters for the correlation
		period_ratio = dummy[1][1:]/dummy[1][:-1]
		for i in range(len(period_ratio[1:])):
			period_ratio[1 + i] = np.random.normal(loc = period_ratio[0]*mu, size = 1, scale = sigma)
		for i in range(len(dummy[1][2:])):
			dummy[1][2+i] = period_ratio[1 + i] * dummy[1][1+i]

		return dummy 

	def weiss_separation_corr_kernel(self, dummy, kernel):
		"""
		Recomputes the planet radii in a multi-planet system according to the correlations
		found by Weiss et al. 2018 using a pdf according to kernel density estimation
		(https://arxiv.org/abs/1706.06204)

		Parameters : 
		dummy : array of arrays, dummy[0] is the array with the radii and dummy[1] the array
		with the orbital periods.
		kernel : the kernel density pdf

		THIS FUNC IS OBSOLETE AND NEEDS TO BE CHANGED
		"""

		period_ratio = np.zeros(len(dummy[1] -1))
		period_ratio[0] = kernel.resample(1)[0]
		for i in range(len(period_ratio)):
			next_ratio = kernel.resample(1)[0]
			period_ratio[1 + i] = next_ratio*period_ratio[i]
		for i in range(len(dummy[1][1:])):
			dummy[1][1+i] =  period_ratio[0 + i] * dummy[1][0+i]
			print("inner P : " + str(dummy[1][1+i]) + " | outer P : " + str(dummy[1][2+i]) + " ratio used : " + str(period_ratio[1+i]))

		return dummy		

	def weiss_period_corr(self, dummy):
		"""
		Recomputes the planet radii in a multi-planet system according to the correlations
		found by Weiss et al. 2018
		(https://arxiv.org/abs/1706.06204)
		...
		mu, skew, sigma = 1.28170899, 9.35402502, 0.91055566 #Parameters for the correlation, using a skew gaussian
		for i in range(len(dummy[1][1:])):
			next_period = scp_stats.skewnorm(skew, loc = mu, scale = sigma).rvs()
			dummy[1][i+1] = next_period*dummy[1][i]
		return dummy 

	def weiss_period_corr_kernel(self, dummy, kernel):
		"""
		Recomputes the planet radii in a multi-planet system according to the correlations
		found by Weiss et al. 2018 using kernel density estimator pdf
		(https://arxiv.org/abs/1706.06204)
		...
		for i in range(len(dummy[1][1:])):
			next_period = kernel.resample(1)[0]
			if(next_period <= 1):
				dummy[1][i+1] = dummy[1][i]
				dummy[1][i] = next_period*dummy[1][i+1]
			else:
				dummy[1][i+1] = next_period*dummy[1][i]
		return dummy 		

	def check_stability_criterion(self, M_star, a_in, a_out, m_in, m_out,  e_in, e_out, threshold):
		"""
		Applies the stability criterion used in He et al where systems that contain planet pairs with mutual Hill radius smaller than a given threshold are discarded.

		Parameters :
		M_star : mass of the star in solar masses
		a_in : semi-major axis of the inner planet in the pair in AU
		a_out : semi-major axis of the outer planet of the pair in AU
		m_in . mass of the inner planet of the pair in Earth mass
		m_out : mass of the outer planet of the pair in Earth mass
		e_in : eccentricity of the inner planet of the pair
		e_out : eccentricity of the outer planet of the pair
		threshold : the mutual hill radius value above which we accept systems as stable

		Returns : A boolean value. True is returned if the pair is seen as stable, otherwise False is returned.
		"""
		m_in = m_in * 5.972E+24
		m_out = m_out * 5.972E+24 #getting the planet masses in kg
		M_star = M_star * 1.989E+30 # getting the star mass in kg
		hill_radius = (a_in+a_out)/2. * ((m_in + m_out)/(3.*M_star))**(1./3.)
		#print(hill_radius)
		delta = (a_out*(1.-e_out) - a_in*(1.+e_in))/hill_radius
		#print(delta)
		return delta > threshold
	def check_system_stability(self, M_star, dummy, planet_masses, planet_ecc, threshold):
		"""
		Check the stability of a planetary system by calling the check_stability_criterion function

		Parameters : 
		dummy : array of arrays, dummy[0] is the array with the radii and dummy[1] the array
		with the orbital periods.
		M_star : mass of the host star in solar masses
		planet_masses : the masses of the planets in earth masses
		planet_ecc : the eccentricities of the planets of the system
		threshold : the value of mutual hill radius above which we accept the syst
		"""
		#Converting periods to semi major axis
		periods = dummy[1]/365.2422 # periods are now in years
		a_planets = np.power(periods, 2./3.)*M_star**(1./3.)#a_planets is now semi major axis in AU
		stability = True
		for it in range(len(dummy[1]) - 1):#We check that each pair of the system is stable
			stability = self.check_stability_criterion(M_star, a_planets[it], a_planets[it + 1], planet_masses[it], planet_masses[it + 1], planet_ecc[it], planet_ecc[it + 1], threshold)
			if(stability == False):
				return False
		return True
...
class flux():
	...
	def __init__(self,
		...
		self.ntelescope = n_telescope

		self.radius_telescope = radius_telescope
	...
	def bb_therm_p_photons(self,
		...
		return 	1E-6*(2.*np.pi*c/(lam*1E-6)**4./(np.exp(h*c/(lam*1E-6*kB*self.Tp))-1.)*((self.Rp*Rearth)/(self.dist*pc))**2.)
	...
	def bb_therm_p_filter_photons(self):
		"""
		Computes planet thermal blackbody flux observed through filter (in
		photons per second per square meter per micron)
		"""
		...
		# Return planet thermal blackbody flux observed through filter
		return self.bb_therm_p_photons(self.nodes)*self.trans
	...
	def bb_therm_p_photons_int(self,
		...
		interferometer loss and the throughput
		"""
	...
	def compute_transmission(self,
		...
		if(self.rp <= 1.643):
			...
			for it in range (len(real_trans[0])):
			    if(it == 0):
			        real_trans[:,it] = transmission_curve[:,0]
			    else:
			        real_trans[:,it] = transmission_curve[:,2]
			for it, wave in enumerate(self.nodes):
			    curve_idx = self.find_nearest(transmission_wavelengths, wave) + 1
			    curve = real_trans[:,curve_idx]
			    if self.rp not in real_trans[:,0]:
			        interpolated_curve = interpolate.interp1d(transmission_curve[:,0], curve, fill_value = "extrapolate")
...
"""
	...
	def refl_p(self,
		...
		if (self.mission == 'MIR' or self.mission == 'METIS'):
